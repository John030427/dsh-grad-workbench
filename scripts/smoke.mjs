#!/usr/bin/env node
/**
 * Post-boot smoke probe for the grad profile.
 * Usage: node scripts/smoke.mjs [port]   (default 3081)
...
await check('GET /plugins/dsh-grad-workbench/client.js → ModuleLoader bundle', async () => {
  const res = await fetch(`${base}/plugins/dsh-grad-workbench/client.js`)
  assert(res.ok, `status ${res.status}`)
